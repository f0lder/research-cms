...
import { Block, PageStatus, ContentTypeDefinition, FieldType } from '@research-cms/shared-types';
import { extractParam, adminRoutes } from '@/lib/utils';
import { getSchema, getAllSchemas, getClientPage, createClientPage, updateClientPage } from '@/app/actions';
...
function FieldBlockDisplay({ block }: { block: AdminBlock & { type: 'field' } }) {
  const typeLabel = {
    [FieldType.TEXT]: 'Text',
  ...
  return (
    <div className="flex gap-2 flex-1 items-center text-sm">
      <span className="text-zinc-600">{block.label}</span>
      <span className="text-[10px] text-zinc-400 font-mono">{block.fieldName}</span>
...
function SortableBlock({
  block, index, schemas, onChange, onRemove, isFieldBlock,
}: {
  block: AdminBlock;
  index: number;
  ...
  return (
    <div
      ...
      className={`border mb-3 ${isFieldBlock ? 'border-blue-200 bg-blue-50' : 'border-zinc-200 bg-white'}`}
    >
      {/* Block header */}
      ...
      <div className="p-3">
        ...
        {block.type === 'field' && <FieldBlockDisplay block={block} />}
      </div>
    </div>
  );
}
...
export default function EntryDetailLayoutPage() {
  ...
  const load = useCallback(async () => {
    const [schemasRes, schemaRes] = await Promise.all([
      getAllSchemas(),
    ...
    // Try to load existing entry detail template
    const templatePageId = `detail-${schemaSlug}`;
    const templateRes = await getClientPage(clientId, templatePageId).catch(() => ({ data: null, error: null }));

    // Load static blocks from template if it exists
    const existingStaticBlocks = (templateRes.data?.blocks ?? []).filter((b): b is AdminBlock => b.type !== 'field');

    // Generate field blocks from schema
    const fieldBlocks: AdminBlock[] = (schemaRes.data.fields ?? [])
      .map((f, i) => ({
        type: 'field' as const,
        fieldName: f.name,
        label: f.label,
    ...
    // Combine static blocks + field blocks
    const combined = [
      ...existingStaticBlocks,
      ...fieldBlocks,
    ];
  ...
  const addBlock = (type: AdminBlock['type']) => {
    if (type === 'field') return; // Can't add field blocks manually
  ...
  const handleSave = async () => {
    ...
    const templatePageId = `detail-${schemaSlug}`;
    const payload = {
      title: `${schema.name} Detail Template`,
      slug: templatePageId,
      status: 'published' as PageStatus,
      type: 'entryDetail' as const,
      schemaSlug,
      blocks,
    };

    const existingRes = await getClientPage(clientId, templatePageId).catch(() => ({ data: null }));
    
    if (existingRes.data && existingRes.data._id) {
      const { error: err } = await updateClientPage(clientId, existingRes.data._id, payload);
      setSaving(false);
      if (err) { setError(err); return; }
    } else {
      const { error: err } = await createClientPage(clientId, payload);
      setSaving(false);
      if (err) { setError(err); return; }
    }
  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="page-heading">{schema?.name} Detail Template</h1>
          <p className="page-sub font-mono text-sm">
            {blocks.length} block{blocks.length !== 1 ? 's' : ''} · drag to reorder static blocks
          </p>
        </div>
        <button
          onClick={handleSave}
      ...
      <div className="mt-6 flex gap-2">
        {(['heading', 'text', 'archive'] as const).map(t => (
      ...
      <div className="mt-8 p-4 bg-blue-50 border border-blue-200 rounded text-sm font-mono">
        <p className="font-semibold text-blue-900 mb-2">Field Blocks (read-only)</p>
        <p className="text-blue-700">
          The fields from your schema are automatically included below static blocks. Their order and visibility
          can be customized by adding/removing them. Static blocks serve as headers, descriptions, or related content sections.
        </p>
      </div>
    </div>
  );
