...
/**
 * Overlays client-specific visible/order preferences onto the schema-derived base blocks.
 * The base is authoritative for field existence (already synced with the live schema).
 */
...
export default function ClientLayoutEditorPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [clientRes, schemaRes, baseLayoutRes] = await Promise.all([
      getClient(id),
      getSchema(slug),
      getLayout(slug), // schema-derived defaults
    ]);
    ...
    if (!clientRes.data || !schemaRes.data) {
      setError('Client or schema not found');
      setLoading(false);
      return;
    }
    ...
    // Base blocks are the schema-derived defaults (all fields, default visibility)
    const baseBlocks = baseLayoutRes.data?.blocks ?? [];

    // If this client already has a saved layout, overlay its preferences on top
    const clientSavedLayout = clientDoc.layouts.find(l => l.schemaSlug === slug);
    const merged = clientSavedLayout
      ? mergeLayouts(baseBlocks, clientSavedLayout.blocks)
      : baseBlocks;
        ...
        <div>
          ...
          {!hasCustom && (
            <p className="text-[11px] text-zinc-400 mt-1">
              Not yet customised — saving will create a layout for this client.
            </p>
          )}
        </div>
        <button
